...
local function HandleIncomingMessage(rawMessage)
    ...
    -- Long message part
    Debug("Received part", partID, "of", totalParts, "for message ID", msgID, "payload length:", #payload)
    
    -- Initialize message tracking
    if not AMS.pendingMessages[msgID] then
        Debug("Creating new message tracking for ID", msgID)
        ...
        -- Set a timeout to clean up incomplete messages after 10 seconds
        C_Timer.After(10, function()
            if AMS.pendingMessages[msgID] then
                local msgData = AMS.pendingMessages[msgID]
                Debug("TIMEOUT: Message ID", msgID, "incomplete after 10s. Received", msgData.receivedParts, "of", msgData.totalParts, "parts")
                -- Log which parts we got
                local gotParts = {}
                for i = 1, msgData.totalParts do
                    if msgData.parts[i] then
                        table.insert(gotParts, i)
                    end
                end
                Debug("TIMEOUT: Got parts:", table.concat(gotParts, ", "))
    ...
    if not msgData.parts[partID] then
        ...
        Debug("Stored part", partID, "- now have", msgData.receivedParts, "of", msgData.totalParts, "parts")
    else
        Debug("WARNING: Duplicate part", partID, "received for message ID", msgID)
    end
    
    -- Check if we have all parts
    if msgData.receivedParts == msgData.totalParts then
        Debug("Message ID", msgID, "complete! Reassembling", msgData.totalParts, "parts...")
...
local function ProcessMessage(serializedMessage)
    ...
    Debug("Deserialize result - success:", tostring(success), "type:", type(blocks))
    
    if not success then
        Debug("ERROR: Smallfolk.loads failed:", tostring(blocks))
        return
    end
    
    if type(blocks) ~= 'table' then
        Debug("ERROR: Expected table, got:", type(blocks))
        return
    end
...
eventFrame:SetScript("OnEvent", function(self, event, prefix, message, channel, sender)
    ...
    Debug("RAW addon message - channel:", channel, "sender:", sender or "nil", "len:", #message)
    
    -- NOTE: We can't filter by sender because server messages appear to come from the player
    -- Instead, we accept all messages and let handler routing sort it out
    -- (handlers only exist for INCOMING message types, not OUTGOING ones)
