    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
                ...
                (ai->IsInBotParty(u->GetVictim()) || me->GetVictim() == u))
            {
                ...
                if (checksteal)
                {
                    if (u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(30449, me->GetMap()->GetDifficultyID()), me))
                        return false; //immune to steal
                }
                else if (ai->GetBotClass() == BOT_CLASS_SHAMAN)
                {
                    if (u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(370, me->GetMap()->GetDifficultyID()), me))
                ...
                for (Unit::AuraApplicationMap::const_iterator itr = AurApps.begin(); itr != AurApps.end(); ++itr)
                {
                    ...
                    if (itr->second->IsPositive() && Info->Dispel == DISPEL_MAGIC &&
                        !(Info->Attributes & (SPELL_ATTR0_PASSIVE | SPELL_ATTR0_DO_NOT_DISPLAY_SPELLBOOK_AURA_ICON_COMBAT_LOG)) &&
                ...
                //        id == 50447 || id == 50448 || id == 50449) //Bloody Vengeance
                //        continue;
                //    if (Info->Attributes & (SPELL_ATTR0_PASSIVE | SPELL_ATTR0_DO_NOT_DISPLAY_SPELLBOOK_AURA_ICON_COMBAT_LOG)) continue;
...
class PolyUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_MAGE && !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(118, me->GetMap()->GetDifficultyID()), me))//Polymorph
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_SHAMAN && !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(51514, me->GetMap()->GetDifficultyID()), me))//Hex
                return true;
...
class FearUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (const_cast<Unit*>(u)->GetDiminishing(DIMINISHING_DISORIENT) > DIMINISHING_LEVEL_3)
                return false;
            if (u->GetReactionTo(me) > REP_NEUTRAL)
                return false;

            if (me->ToCreature()->GetBotClass() == BOT_CLASS_WARLOCK &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(5782, me->GetMap()->GetDifficultyID()), me))//fear rank1
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(1513, me->GetMap()->GetDifficultyID()), me))//scare beast rank1
                return true;
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_PALADIN && me->GetDistance(u) < 10)//prevent break due to AOE damage
                return false;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_PALADIN &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(20066, me->GetMap()->GetDifficultyID()), me))//repentance
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(60210, me->GetMap()->GetDifficultyID()), me))//freezing arrow effect
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(19386, me->GetMap()->GetDifficultyID()), me))//wyvern sting rank 1
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(1991, me->GetMap()->GetDifficultyID()), me))//scatter shot
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_ROGUE &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(2094, me->GetMap()->GetDifficultyID()), me))//blind
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_WARLOCK &&
                u->GetCreatureType() == CREATURE_TYPE_HUMANOID &&
                (u->GetVictim() || u->IsControlledByPlayer()) &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(6358, me->GetMap()->GetDifficultyID()), me))//seduction
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_DREADLORD &&
                u->GetCreatureType() != CREATURE_TYPE_UNDEAD &&
                !u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(SPELL_SLEEP, me->GetMap()->GetDifficultyID()), me))
                return true;
...
class UndeadCCUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (u->GetReactionTo(me) > REP_NEUTRAL)
                return false;
            if (u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(m_spellId, me->GetMap()->GetDifficultyID())->TryGetSpellInfoOverride(me), me))
                return false;
            if (m_ai->IsPointedNoDPSTarget(u) && bot_ai::IsDamagingSpell(sSpellMgr->GetSpellInfo(m_spellId, me->GetMap()->GetDifficultyID())->TryGetSpellInfoOverride(me)))
                return false;
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (m_spell)
            {
                ...
                SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(m_spell, me->GetMap()->GetDifficultyID())->TryGetSpellInfoOverride(me);
                ...
                if (!CastInterruptionCheck(u, spellInfo, me))
                    return false;
            }
        ...
        static bool CastInterruptionCheck(Unit const* u, SpellInfo const* spellInfo, Unit const* me)
        {
            if (spellInfo->HasEffect(SPELL_EFFECT_INTERRUPT_CAST) && spellInfo->GetFirstRankSpell()->Id != 853) //hammer of justice
            {
                ...
                for (uint8 i = CURRENT_FIRST_NON_MELEE_SPELL; i != CURRENT_AUTOREPEAT_SPELL; ++i)
                {
                    ...
                    //     ((i == CURRENT_GENERIC_SPELL && curSpell->GetSpellInfo()->InterruptFlags & SpellInterruptFlags::Combat) ||
                    //     (i == CURRENT_CHANNELED_SPELL && curSpell->GetSpellInfo()->ChannelInterruptFlags & SpellInterruptFlags::Combat))))
                    //     return false;
                    if ((curSpell->getState() == SPELL_STATE_CASTING
                        || (curSpell->getState() == SPELL_STATE_PREPARING && curSpell->GetCastTime() > 0.0f))
                        && spellInfo->CanBeInterrupted(u, me))
                        return false;
                }
            }
...
class TranquilTargetCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            {
                if (u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(19801, me->GetMap()->GetDifficultyID()), me))
...
class FarTauntUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (!ai->IsInBotParty(u->GetVictim()))
                return false;
            if (ai->GetBotClass() == BOT_CLASS_WARRIOR && u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(355, me->GetMap()->GetDifficultyID()), me))
                return false; //taunt
            else if (ai->GetBotClass() == BOT_CLASS_PALADIN && u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(62124, me->GetMap()->GetDifficultyID()), me))
                return false; //HoR
            else if (ai->GetBotClass() == BOT_CLASS_DRUID && u->IsImmunedToSpell(sSpellMgr->GetSpellInfo(6795, me->GetMap()->GetDifficultyID()), me))
                return false; //Growl
...
public:
    ...
    bool operator()(WorldObject const* ob)
    {
        ...
        else if (Creature const* cre = ob->ToCreature())
        {
            if (
                cre->IsVisible() 
                && cre->getDeathState() == CORPSE
                && cre->HasUnitFlag(UNIT_FLAG_SKINNABLE)
                && cre->IsFullyLooted()
                && cre->isTappedBy(_checker->ToPlayer())
                && CheckSkill(
                    cre->GetCreatureDifficulty()->GetRequiredLootSkill()
                )
            )
            {
