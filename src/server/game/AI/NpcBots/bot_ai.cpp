...
static constexpr GossipOptionNpc BOT_ICON_ON = GossipOptionNpc::Battlemaster;
static constexpr GossipOptionNpc BOT_ICON_OFF = GossipOptionNpc::None;
...
uint8 GroupIconsFlags[TargetIconNamesCacheSize] =
{
    /*STAR        = */0x001,
    /*CIRCLE      = */0x002,
    /*DIAMOND     = */0x004,
...
const std::string bot_ai::LocalizedNpcText(Player const* forPlayer, uint32 textId)
{
    ...
    // @todo jh this is wonky, they appear to have simplified, not sure this is gonna work
    ...
    if(NpcText const* nt = sObjectMgr->GetNpcText(textId))
    {
            BroadcastTextEntry const* bct = sBroadcastTextStore.LookupEntry(nt->Data[0].BroadcastTextID);
            std::string npcTextString(DB2Manager::GetBroadcastTextValue(bct, loc, forPlayer ? forPlayer->GetGender() : GENDER_MALE));
            // return std::string(DB2Manager::GetBroadcastTextValue(bct, loc, forPlayer ? forPlayer->GetGender() : GENDER_MALE));
            return npcTextString;
    }
...

    {
        ...
        if (!unk_botstrings.count(textId))
        {
            ...
            std::string msg = "<unlocalized string ";
            if (loc == DEFAULT_LOCALE)
            {
                msg = "<undefined string ";
            }
            msg += std::to_string(textId) + ">";
            unk_botstrings[textId] = msg;
        }
...
SpellCastResult bot_ai::CheckBotCast(Unit const* victim, uint32 spellId) const
{
    ...
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId, victim->GetMap()->GetDifficultyID());
    if (!spellInfo)
        return SPELL_FAILED_DONT_REPORT;
    ...
    // @todo jh this needs to iterate over all of the powers that are returned from CalcPowerCost
    if (me->GetPower(static_cast<Powers>(spellInfo->PowerCosts[0]->PowerType)) < spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask())[spellInfo->PowerCosts[0]->PowerType].Amount)
        ...
        (victim->GetLevel() + 10) < int32(spellInfo->GetFirstRankSpell()->SpellLevel))
    {
        ...
        {
            if (spellInfo->GetEffects()[i].Effect == SPELL_EFFECT_APPLY_AURA && spellInfo->IsPositiveEffect(i))
            {
    ...
    switch (_botclass)
    {
        ...
        case BOT_CLASS_ROGUE: //Killing Spree
            if (me->HasAuraType(SPELL_AURA_DISABLE_CASTING_EXCEPT_ABILITIES))
                return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;
            break;
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, TriggerCastFlags flags)
{
    ...
    m_botSpellInfo = sSpellMgr->GetSpellInfo(spellId, victim->GetMap()->GetDifficultyID());
    if (!m_botSpellInfo)
        return false;
    ...
    if (victim->isType(TYPEMASK_UNIT))
    {
        ...
        if (!m_botSpellInfo->IsTargetingArea())
        {
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                if (m_botSpellInfo->GetEffects()[i].IsAura())
                    approximateAuraEffectMask |= 1 << i;
                else if (m_botSpellInfo->GetEffects()[i].IsEffect())
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                if (!nonAuraEffectMask && (approximateAuraEffectMask & (1 << i)))
                    if (!victim->IsHighestExclusiveAuraEffect(m_botSpellInfo, AuraType(m_botSpellInfo->GetEffects()[i].ApplyAuraName),
                        m_botSpellInfo->GetEffects()[i].CalcValue(me, (int32 *)&m_botSpellInfo->GetEffects()[i].BasePoints), approximateAuraEffectMask, false))
                        return false;
            }
        }
    ...
    if (/*victim->isType(TYPEMASK_UNIT) && */!HasBotCommandState(BOT_COMMAND_ISSUED_ORDER) &&
        !CastingUnitCheck::CastInterruptionCheck(victim, m_botSpellInfo, victim))
        return false;
    ...
    if (me->isMoving()
        ...
        && !(m_botSpellInfo->Attributes & SPELL_ATTR0_ON_NEXT_SWING)
        && !m_botSpellInfo->IsAutoRepeatRangedSpell() &&
        (
            (m_botSpellInfo->InterruptFlags & SpellInterruptFlags::Movement) != SpellInterruptFlags::None
    ...
    {
        if (!doMana && me->HasInterruptFlag(SpellAuraInterruptFlags::Standing))
            UpdateMana();
...
void bot_ai::_calculatePos(Unit const* followUnit, Position& pos, float* speed/* = nullptr*/) const
{
    ...
    {
        // This was barfing because mmover is const but
        // GetFirstCollisionPosition is not const. This is probably
        // not a good way to fix this issue.
        // @todo jh look into this.
        Unit* ncmover = const_cast<Unit*>(mmover);
        mpos = ncmover->GetFirstCollisionPosition(mydist+size, angle);
    }
...
void bot_ai::BotMovement(BotMovementType type, Position const* pos, Unit* target, bool generatePath, float speed) const
{
    ...
    switch (type)
    {
        case BOT_MOVE_CHASE:
            ...
            break;
        case BOT_MOVE_POINT:
            // @todo jh this was PointMovementGenerator<Creature> to use that
            // template but that wouldn't compile. Removing the template
            // got it to compile but does that break something?
            mover->GetMotionMaster()->Add(
...
bool bot_ai::IsPointedTarget(Unit const* target, uint8 targetFlags) const
{
    if (Group const* gr = (IAmFree() ? nullptr : master->GetGroup()))
        if (targetFlags)
            for (uint8 i = 0; i != TargetIconNamesCacheSize; ++i)
...
void bot_ai::BuffAndHealGroup(uint32 diff)
{
    ...
    if (HasRole(BOT_ROLE_HEAL))
    {
        ...
        for (uint8 i = 0; i != TargetIconNamesCacheSize; ++i)
        {
            ...
            {
                ObjectGuid guid = pGroup->GetTargetIcons()[i];
                if (!guid.IsEmpty())
                {
...
void bot_ai::ResurrectGroup(uint32 spell_id)
{
    ...
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spell_id, me->GetMap()->GetDifficultyID());
    ASSERT(spellInfo);
    spellInfo = spellInfo->TryGetSpellInfoOverride(me);
    if (int32(me->GetPower(static_cast<Powers>(spellInfo->PowerCosts[0]->PowerType))) < spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask())[spellInfo->PowerCosts[0]->PowerType].Amount)
        return;
...
bool bot_ai::_canCureTarget(Unit const* target, uint32 cureSpell) const
{
    ...
    SpellInfo const* info = sSpellMgr->GetSpellInfo(cureSpell, me->GetMap()->GetDifficultyID());
    if (!info)
        return false;
    ...
    uint32 dispelMask = 0;
    for (uint8 i = 0; i != MAX_SPELL_EFFECTS; ++i)
        if (info->GetEffects()[i].Effect == SPELL_EFFECT_DISPEL)
            dispelMask |= SpellInfo::GetDispelMask(DispelType(info->GetEffects()[i].MiscValue));
...
void bot_ai::_getBotDispellableAuraList(Unit const* target, uint32 dispelMask, std::list<Aura const*> &dispelList) const
{
    ...
    if ((dispelMask & (1<<DISPEL_DISEASE)) &&
        target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DEATHKNIGHT, 1494, ObjectGuid::Empty))
    ...
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if (aura->GetSpellInfo()->GetDispelMask() & dispelMask)
        {
            ...
            if (HasRole(BOT_ROLE_HEAL) && aura->GetSpellInfo()->IsRankOf(sSpellMgr->GetSpellInfo(34914, target->GetMap()->GetDifficultyID())))
                continue;
...
void bot_ai::_listAuras(Player const* player, Unit const* unit) const
{
    ...
    for (Unit::AuraMap::const_iterator itr = vAuras.begin(); itr != vAuras.end(); ++itr)
    {
        ...
        SpellInfo const* learnSpellInfo = sSpellMgr->GetSpellInfo(
            spellInfo->GetEffects()[0].TriggerSpell,
            unit->GetMap()->GetDifficultyID()
        );

        const LocalizedString* spellName = spellInfo->SpellName;
        const std::string name = spellName->Str[locale];

        botstring << "\n" << id << " - |cffffffff|Hspell:" << id << "|h[" << name;
        botstring << ' ' << localeNames[locale] << "]|h|r";

        // @todo jh all this talent stuff is likely very different now
        // removing it but likely this is going to handicap bots.
        // uint32 talentcost = GetTalentSpellCost(id);
        uint32 rank = 0;
        // if (talentcost > 0 && (spellInfo->GetNextRankSpell() || spellInfo->GetPrevRankSpell()))
        //     rank = talentcost;
        // else
        if (learnSpellInfo && (spellInfo->GetNextRankSpell() || spellInfo->GetPrevRankSpell()))
        ...
        if ((spellInfo->Attributes & SPELL_ATTR0_DO_NOT_DISPLAY_SPELLBOOK_AURA_ICON_COMBAT_LOG) ||
...
//nbcbot    botstring << "\n" << LocalizedNpcText(player, BOT_TEXT_BLOCKVALUE) << ": " << uint32(unit->GetShieldBlockValue());
    ...
    unit->CalculateMinMaxDamage(BASE_ATTACK, true, true, minDamage, maxDamage);

    WeaponAttackType type = BASE_ATTACK;
    float attSpeed = (unit->GetBaseAttackTime(type) * unit->m_modAttackSpeedPct[type])/1000.f;

    botstring << "\n" << LocalizedNpcText(player, BOT_TEXT_DMG_RANGE_MAINHAND) << ": " 
        << LocalizedNpcText(player, BOT_TEXT_MIN) << ": " << minDamage << ", " 
        << LocalizedNpcText(player, BOT_TEXT_MAX) << ": " << (maxDamage + 1.f);
    ...
    botstring << "\n" << LocalizedNpcText(player, BOT_TEXT_ATTACK_TIME_MAINHAND) << ": " << float(attSpeed)
        << " (" << float(((minDamage + maxDamage) / 2) / attSpeed) << " " << LocalizedNpcText(player, BOT_TEXT_DPS) << ")";
    ...
    if (unit->haveOffhandWeapon())
    {
        ...
        attSpeed = (unit->GetBaseAttackTime(type) * unit->m_modAttackSpeedPct[type])/1000.f;
        unit->CalculateMinMaxDamage(type, true, true, minDamage, maxDamage);
        ...
        botstring << "\n" << LocalizedNpcText(player, BOT_TEXT_DMG_RANGE_OFFHAND) << ": " << LocalizedNpcText(player, BOT_TEXT_MIN) << ": " 
            << int32(minDamage) << ", " << LocalizedNpcText(player, BOT_TEXT_MAX) << ": " 
            << int32(maxDamage + 1.f);
        //end npcbot
    ...
    Item const* rangedItem = _equips[BOT_SLOT_RANGED]; //->ItemEntry[BOT_SLOT_RANGED];
    ...
    )
    {
        ...
        botstring << "\n" << LocalizedNpcText(player, BOT_TEXT_DMG_RANGE_RANGED) << ": " 
            << LocalizedNpcText(player, BOT_TEXT_MIN) << ": " << int32(minDamage) << ", " 
            << LocalizedNpcText(player, BOT_TEXT_MAX) << ": " << int32(maxDamage + 1.f);
    ...
    if (unit == me)
    {
        ...
        if (me->GetMaxPower(POWER_MANA) > 1)
        {
            ...
            botstring << "\n" << LocalizedNpcText(player, BOT_TEXT_REGEN_MP_CAST) << ": " << float((_botclass == BOT_CLASS_SPHYNX ? -1.f : 1.f) 
                * me->m_unitData->PowerRegenInterruptedFlatModifier[POWER_MANA] * sWorld->getRate(RATE_POWER_MANA) * 5.0f);
            botstring << "\n" << LocalizedNpcText(player, BOT_TEXT_REGEN_MP_NOCAST) << ": " << float((_botclass == BOT_CLASS_SPHYNX ? -1.f : 1.f) 
                * me->m_unitData->PowerRegenFlatModifier[POWER_MANA] * sWorld->getRate(RATE_POWER_MANA) * 5.0f);
            //end npcbot
...
void bot_ai::SetStats(bool force)
{
    ...
    if (IsWanderer())
    {
        if (_baseLevel == 0) //this only happens once
        {
            ...
            //@todo jh this needs to go back to something a bit more random
            //mylevel = urand(me->GetCreatureTemplate()-> minlevel, me->GetCreatureTemplate()->maxlevel);
            mylevel = urand(1, DEFAULT_MAX_LEVEL);

            uint32 creatureClass = (uint32)me->GetCreatureTemplate()->Classification;
            mylevel += BotDataMgr::GetLevelBonusForBotRank(creatureClass);
        ...
        else if (me->GetMap()->GetEntry()->IsContinent())
        {
            ...
            uint32 mapLevelBonus = BotDataMgr::GetLevelBonusForBotRank((uint32)me->GetCreatureTemplate()->Classification);
            mapmaxlevel += mapLevelBonus;
    ...
    {
        uint32 creatureLevel = (uint32)me->GetCreatureTemplate()->Classification;
        uint32 levelBonus = BotDataMgr::GetLevelBonusForBotRank(creatureLevel);
        mylevel += levelBonus;
    }
    ...
    if (_botclass == BOT_CLASS_DRUID && RespectEquipsAttackTime())
    {
        ...
        me->setAttackTimer(BASE_ATTACK, delay);
    }
    ...
    Unit::AuraEffectList const& mAPbyStat = me->GetAuraEffectsByType(SPELL_AURA_MOD_ARMOR_PCT_FROM_STAT);
    ...
    atpower += me->GetTotalAuraModifier(SPELL_AURA_LINKED_2);
    ...
    tempval += me->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_RATING, (1 << CR_RESILIENCE_CRIT_TAKEN) | (1 << CR_RESILIENCE_PLAYER_DAMAGE) | (1 << CR_LIFESTEAL));
    value += tempval * std::max<float>(_getRatingMultiplier(CR_RESILIENCE_CRIT_TAKEN), std::max<float>(_getRatingMultiplier(CR_RESILIENCE_PLAYER_DAMAGE), _getRatingMultiplier(CR_LIFESTEAL)));
    ...
    if (CanCrit())
    {
        ...
        // @todo jh This needs to actually use bot crit extras rather than just defaults
        ...
        if (_botclass == BOT_CLASS_MAGE && me->HasAuraTypeWithFamilyFlags(SPELL_AURA_MOD_STAT, SPELLFAMILY_MAGE, 0x40000))
        ...
        if (_botclass == BOT_CLASS_ROGUE)
        {
            ...
            if (mylevel >= 20)
            {
                ...
                {
                    if (
                        mainhand->GetTemplate()->GetClass() == ITEM_CLASS_WEAPON &&
                        (
                            mainhand->GetTemplate()->GetSubClass() == ITEM_SUBCLASS_WEAPON_DAGGER ||
                            mainhand->GetTemplate()->GetSubClass() == ITEM_SUBCLASS_WEAPON_FIST_WEAPON
                        )
                    )
                        value += 5.f;
                }
    ...
    if (CanDodge())
    {
        ...
        // @todo jh this needs to be updated to use actual dodge from agility
    ...
    if (IsCastingClass(_botclass))
    {
        ...
        if (_botclass == BOT_CLASS_PRIEST && mylevel >= 30)
        {
            ...
            if (mylevel >= 30 &&
                me->GetAuraEffect(
                    ...
                    SPELLFAMILY_GENERIC,
                    // @todo jh this is likely not the value we want here
                    1499,
                    me->GetGUID()
                )
            )
        ...
        if (_botclass == BOT_CLASS_WARLOCK)
        {
            if (me->GetAuraEffect(SPELL_AURA_MOD_SPELL_DAMAGE_OF_STAT_PERCENT, SPELLFAMILY_WARLOCK, 0x20000000, me->GetGUID()))
            {
            ...
            // @todo jh the 208 in this is likely not correct, in the old code it refers to an iconid
            // but in the new code it's some sort of familyFlag. See Unit.cpp line 4678 for GetAuraEffect
            if (me->GetAuraEffect(SPELL_AURA_MOD_SPELL_DAMAGE_OF_STAT_PERCENT, SPELLFAMILY_WARLOCK, 208, me->GetGUID()))
        ...
        if (_botclass == BOT_CLASS_SPHYNX)
        {
            ...
            for (uint8 i = BOT_SLOT_MAINHAND; i <= BOT_SLOT_OFFHAND; ++i)
                if (ItemTemplate const* proto = _equips[i] ? _equips[i]->GetTemplate() : nullptr)
                    value += proto->GetDPS(_equips[i]->GetItemLevel(me)) * 1.35f;
        }
        if (_botclass == BOT_CLASS_ARCHMAGE)
        {
...
bool bot_ai::IsInBotParty(Unit const* unit) const
{
    ...
    if (IAmFree())
    {
        ...
        if (me->HasPvpFlag(UNIT_BYTE2_FLAG_FFA_PVP) ||
            unit->HasPvpFlag(UNIT_BYTE2_FLAG_FFA_PVP)
        )
            return false;
    ...
    if (Group const* gr = master->GetGroup())
    {
        ...
        for (uint8 i = 0; i != TargetIconNamesCacheSize; ++i)
            if ((BotMgr::GetHealTargetIconFlags() & GroupIconsFlags[i])
                && !(
                    (
                        BotMgr::GetOffTankTargetIconFlags() 
                        | BotMgr::GetDPSTargetIconFlags() 
    ...
    {
        ObjectGuid ownerGuid = unit->GetOwnerGUID().IsPlayer() 
            ? unit->GetOwnerGUID() : unit->GetCreatorGUID().IsPlayer() 
                ? unit->GetCreatorGUID() : ObjectGuid::Empty;

        // if we are in a vehicle get the vehicle driver/owner
        ...
        if (ownerGuid.IsPlayer() && master->GetBotMgr()->GetBot(ownerGuid))
            return true;

        //controlled by group member
        //pets, minions, guardians etc.
        //bot pets too
        if (ownerGuid.IsCreature())
            if (Group const* gr = master->GetGroup())
...
bool bot_ai::IsInBotParty(ObjectGuid guid) const
{
    ...
    if (Group const* gr = master->GetGroup())
    {
        ...
        for (uint8 i = 0; i != TargetIconNamesCacheSize; ++i)
            if (
                (
                    BotMgr::GetHealTargetIconFlags() & GroupIconsFlags[i]
                ) && !(
                    (
                        BotMgr::GetOffTankTargetIconFlags() | BotMgr::GetDPSTargetIconFlags()
                    )
                    & GroupIconsFlags[i]
                )
            )
            {
                ObjectGuid gguid = gr->GetTargetIcons()[i];
                if (!gguid.IsEmpty())
                    if (gguid == guid)
                        return true;
            }
...
void bot_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    ...
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId, me->ToUnit()->GetMap()->GetDifficultyID());
    if (!spellInfo)
    {
...
Unit* bot_ai::_getVehicleTarget(BotVehicleStrats /*strat*/) const
{
    ...
    if (gr && IsOffTank())
    {
        ...
        for (int8 i = TargetIconNamesCacheSize - 1; i >= 0; --i)
        {
    ...
    if (mytar && mytar->GetTypeId() == TYPEID_UNIT &&
        mytar->ToCreature()->GetCreatureTemplate()->Classification == CreatureClassifications::Elite)
        followdist *= 1.5f;
...
std::tuple<Unit*, Unit*> bot_ai::_getTargets(bool byspell, bool ranged, bool &reset) const
{
    ...
    if (gr && IsOffTank())
    {
        ...
        {
            ObjectGuid guid = gr->GetTargetIcons()[_primaryIconTank];
            if (!guid.IsEmpty())
            {
                if (mytar && mytar->GetGUID() == guid)
                    return { mytar, mytar };
            ...
            if (BotMgr::GetTankTargetIconFlags() & GroupIconsFlags[i])
            {
                ...
                if (!guid.IsEmpty())
                {
                    ...
                    if (Unit* unit = ObjectAccessor::GetUnit(*me, guid))
                    {
                        if (
                            unit->IsVisible()
                            && unit->isTargetableForAttack(false)
                            && me->IsValidAttackTarget(unit)
                            && unit->IsInCombat()
                            && (
                                CanSeeEveryone()
                                || (
                                       me->CanSeeOrDetect(unit) && unit->InSamePhase(me)
                                    )
                                )
                            )
                        {
    ...
    if (gr)
    {
        if (_primaryIconDamage >= 0)
        {
            ...
            {
                ObjectGuid guid = gr->GetTargetIcons()[_primaryIconDamage];
                if (!guid.IsEmpty())
                {
                    if (mytar && mytar->GetGUID() == guid)
                        return { mytar, mytar };
...
bool bot_ai::CheckAttackTarget()
{
    ...
    if (GetEngageTimer() > lastdiff)
        ...
        && mytar != me->GetVictim() 
        && mytar->GetVictim() 
        && mytar->CanHaveThreatList() 
        && mytar->ToCreature()->isWorldBoss()
        && me->GetMap()->IsRaid())
    {
    ...
    switch (sender)
    {
        ...
        case GOSSIP_SENDER_ABILITIES_USAGE_LIST_SUPPORT:
        {
            ...
            for (std::vector<uint32>::const_iterator itr = myspells->begin(); itr != myspells->end(); ++itr)
            {
                ...
                GossipOptionNpc icon = (GetSpell(*itr) != 0) ? BOT_ICON_ON : BOT_ICON_OFF;
        ...
        case GOSSIP_SENDER_SPEC:
        {
            ...
            {
                GossipOptionNpc icon = (GetSpec() == i) ? BOT_ICON_ON : BOT_ICON_OFF;
                ...
                case 2: //Remove a visible buff
                {
                    ...
                    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                    {
                        ...
                        if (info->IsPassive() || !info->IsPositive())
                            continue;
                        if ((info->Attributes & (SPELL_ATTR0_CANT_CANCEL | SPELL_ATTR0_DO_NOT_DISPLAY_SPELLBOOK_AURA_ICON_COMBAT_LOG)) ||
...
//ROLES//
/////////
GossipOptionNpc bot_ai::GetRoleIcon(uint32 role) const
{
