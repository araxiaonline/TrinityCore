...
public:
    ...
    struct warlock_botpetAI : public bot_pet_ai
    {
        ...
        void EnterEvadeMode(EvadeReason why = EvadeReason::Other) override { bot_pet_ai::EnterEvadeMode(why); }
        ...
        void DoPetActions(uint32 diff)
        {
            ...
            else if (myType == BOT_PET_SUCCUBUS)
            {
                ...
                if (GetSpell(SEDUCTION_1) && Rand() < 20 && !IsCasting())
                {
                    ...
                    if (target && target->GetDiminishing(DIMINISHING_DISORIENT) <= DIMINISHING_LEVEL_2 + 1 * (target->IsNonMeleeSpellCast(false, false, true)))
                    {
